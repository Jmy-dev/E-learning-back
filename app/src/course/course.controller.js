        ...
        if(req.user.isAdmin) {
            ...
            if(req.body.videosURL){
                ...
                if(!course.videosURL.includes(req.body.videosURL)) {
                    const updatedCourse = await Course.findByIdAndUpdate({_id:req.params.id} , {$push: {videosURL:req.body.videosURL}} , {new: true})
                    .exec()
            ...
            if(req.body.quizes){
                ...
                if(!course.quizes.includes(req.body.quizes)) {
                    console.log("Hey!!")
                    const updatedCourse = await Course.findByIdAndUpdate({_id:req.params.id} , {$push: {quizes:req.body.quizes}} , {new: true})
                    .exec()
                    if(!updatedCourse) {
                        return res.status(400)
                    }
            ...
            if(req.body.department) {
                ...
                if(!department.courses.includes(req.params.id)) {
                    const updatedDepartment = await Department.updateOne({_id:department._id},{$push: {courses: req.params.id}} , {new: true} );
                ...
                const updatedCourse = await Course.findByIdAndUpdate({_id:req.params.id} , body , {new:true})
                .populate('department')
                .lean()
                .exec()
